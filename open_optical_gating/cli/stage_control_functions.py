...
def init_stage(usb_information):
    """Function to initialise communication with serial stage."""
    ## TODO make clear what this is doing
    ser = serial.Serial(
        usb_information[0],
...
def set_stage_to_recieve_input(ser, address, encoding, terminator):
    ...
    command = (
        str(address) + set_neg_software_limit + new_neg_software_limit + str(terminator)
    )
    ser.write(command.encode(encoding))
...
def move_stage(ser, address, increment, encoding, terminate):
    ...
    if not response:
        ...
        print(
            "Error. No response obtained from stage "
            + str(address)
        ...
        current_position = float(
            re.sub(str(address) + "TP", "", response)
        )  # Extracts position from response
        ...
        if (current_position + float(increment)) > negative_software_limit and (
            current_position + float(increment)
            ...
            print(
                "Error. Cannot move stage by increment "
                + str(increment)
                + " at position "
                + str(current_position)
                + " as would be outside software limits of "
                + str(negative_software_limit)
                + " - "
                + str(positive_software_limit)
            )
...
def set_user_stage_limits(ser, address, encoding, terminator):
    ...
    input(
        "Please move the stage to the edge of the zebrafish heart.\nPress any key when the stage is in the correct positon."
    )
    ...
    # Waits the user to move the stage
    input(
        "Please move the stage to the other edge of the zebrafish heart.\nPress any key when the stage is in the correct positon."
    )
    ...
    print(
        "Limits acquired. The stage will now move to from the one edge of the heart, to the other and then back again.\nPlease check to see if the limits are correct."
    )
    ...
    # Moves the stages
    stage_response, current_position = move_stage(
        ser, address, relative_increment, encoding, terminator
    )
    ...
    correct = input(
        "Were these limits correct?\nPress y for yes or anything else for no."
    )
    # If results are incorrect, relaunches function
    if correct != "y":
        first_limit, second_limit, current_position = set_user_stage_limits(
            ser, address, encoding, terminator
        )
