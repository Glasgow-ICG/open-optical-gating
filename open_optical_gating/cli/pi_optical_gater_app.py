...
class PiOpticalGater(server.OpticalGater):
    ...
    def setup_camera(self):
        ...
        # Setup logging
        logger.remove()
        logger.add("user_log_folder/oog_{time}.log", level= self.settings["general"]["log_level"], format = "{time:YYYY-MM-DD | HH:mm:ss:SSSSS} | {level} | {module}:{name}:{function}:{line} --- {message}")
